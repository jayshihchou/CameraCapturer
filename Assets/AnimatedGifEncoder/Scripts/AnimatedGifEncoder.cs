...
/*
	Modified by Chou Shih-Chieh 4/5/2018
	 */
#endregion
...
//		protected FileStream fs;
		...
		protected Texture2D image; // current frame
...
//			}
			...
			if (transparent != null ) 
			{
                ...
                transIndex = nq.Map((int)(transparent.Value.b * 255f), (int)(transparent.Value.g * 255f), (int)(transparent.Value.r * 255f));
			}
		}
		...
		void ReverseArray(Color32[] array)
		{
			int size = array.Length;
			int centre = size / 2 + (size % 2 != 0 ? 0 : 1);

			Color32 temp;
			for (int c = 0; c < centre; ++c)
			{
				int next = size - 1 - c;
				temp = array[c];
				array[c] = array[next];
		...
		{
			var imgColors = image.GetPixels32();
			//Array.Reverse(imgColors);
			ReverseArray(imgColors);
			...
			for (int y = 0; y < height; ++y)
			{
				...
				{
					Color32 color = imgColors[y * width + x];
